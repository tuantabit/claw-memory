...
import { ClawMemoryCompactor, type CompactionConfig } from "../compactor/index.js";
...
export function createCompactTool(engine: ClawMemoryEngine) {
  return {
    name: "claw-memory_compact",
    description:
    ...
    async execute(input: CompactInput): Promise<CompactOutput> {
      try {
        ...
        if (input.history) {
          const compactor = new ClawMemoryCompactor(db);
        ...
        const compactor = new ClawMemoryCompactor(db, compactionConfig);
