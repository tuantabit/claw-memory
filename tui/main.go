// claw-memory-tui: Terminal UI for ClawMemory-Claw Database
...
func main() {
	...
	flag.StringVar(&cfg.DBPath, "db", defaultDBPath(), "Path to claw-memory database")
	flag.BoolVar(&cfg.Verbose, "verbose", false, "Enable verbose output")
	flag.BoolVar(&showVersion, "version", false, "Show version info")
	flag.Parse()

	if showVersion {
		fmt.Printf("claw-memory-tui %s (commit: %s, built: %s)\n", version, commit, date)
		os.Exit(0)
	}
	...
	if _, err := os.Stat(cfg.DBPath); os.IsNotExist(err) {
		...
		fmt.Fprintf(os.Stderr, "Run claw-memory first to create the database.\n")
		os.Exit(1)
	}
...
func defaultDBPath() string {
	...
	if err != nil {
		return "claw-memory.db"
	}
	return filepath.Join(home, ".openclaw", "claw-memory.db")
}
